...
Product.Config.prototype.fillSelect = function(element){
    ...
    if(options) {
        ...
        for(var i=0;i<options.length;i++){
            ...
            {
                // extension Code  // Paras Code: check if the options[i] has image property set, if not skip the option, the
                //primary swatch is already displayed.
                if (this.config.attributes[attributeId].use_image && options[i].hasOwnProperty('image'))
                {
